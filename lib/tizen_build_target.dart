...
/// See: [IosAssetBundle] in `ios.dart`
abstract class TizenAssetBundle extends Target {
  const TizenAssetBundle(this.project, this.buildInfo);
...
/// See: [ReleaseIosApplicationBundle] in `ios.dart`
class ReleaseApplicationBundle extends TizenAssetBundle {
  ReleaseApplicationBundle(FlutterProject project, TizenBuildInfo buildInfo)
      : super(project, buildInfo);

  @override
  String get name => 'release_tizen_bundle_flutter_assets';
...
/// See: [DebugIosApplicationBundle] in `ios.dart`
class DebugApplicationBundle extends TizenAssetBundle {
  DebugApplicationBundle(FlutterProject project, TizenBuildInfo buildInfo)
      : super(project, buildInfo);

  @override
  String get name => 'debug_tizen_bundle_flutter_assets';
...
class TizenPlugins extends Target {
  ...
  @override
  List<Source> get outputs => const <Source>[
        // TODO(HakkyuKim): Should use depfile generated from tizen tool
      ];
  ...
  Future<void> build(Environment environment) async {
    ...
    final Directory pluginsOutput =
        environment.outputDir.childDirectory('plugins');
    if (pluginsOutput.existsSync()) {
      pluginsOutput.deleteSync(recursive: true);
    }
    pluginsOutput.createSync(recursive: true);
    ...
    for (final TizenPlugin plugin in nativePlugins) {
      ...
      for (final String arch in buildInfo.targetArchs) {
        ...
        sharedLib.copySync(pluginsOutput.childFile(sharedLib.basename).path);
      }
    }
  }
}
...
/// The method that packages dotnet tpk
///
/// This is also responsible for collecting resources before packaging, including:
/// * Copying app.so from the build directory to flutter/ephermeral/lib/arch (release)
/// * Copying the engine binaries to flutter/ephermeral/lib/arch
/// * Copying the plugin binaries to flutter/ephermeral/lib/arch
/// * Copying icu to flutter/ephermeral/res/
/// * Copying the flutter assets to flutter/ephermeral/res/flutter_assets
Future<void> buildDotnetTpk({
  @required FlutterProject project,
  @required TizenBuildInfo buildInfo,
  ...
  for (final String arch in buildInfo.targetArchs) {
    ...
    // Copy plugins to lib
    final Directory pluginsDir = outputDir.childDirectory('plugins');
    if (pluginsDir.existsSync()) {
      globals.fsUtils.copyDirectorySync(pluginsDir, libDir);
    }
...
/// The method that packages native tpk
///
/// This is also responsible for collecting resources before packaging, including:
/// * Copying app.so from the build directory to lib (release)
/// * Copying the engine binaries to lib
/// * Copying the plugin binaries to lib
/// * Copying icu to res
/// * Copying the flutter assets to res/flutter_assets
Future<void> buildNativeTpk({
  @required FlutterProject project,
  @required TizenBuildInfo buildInfo,
  @required Environment environment,
}) async {
  ...
  final Directory resDir = tizenDir.childDirectory('res')
    ..createSync(recursive: true);
  // clear {PROJECT_DIR}/tizen/res/flutter_assets
  ...
  //
  // TODO(HakkyuKim): revist this for a more complete solution
  libDir
      .listSync()
      .whereType<File>()
      .where((File file) => file.path.endsWith('_plugin.so'))
      .toList()
      .forEach((File file) {
  ...
  final Directory commonDir = engineDir.parent.childDirectory('common');
  final Directory clientWrapperDir = commonDir.childDirectory('client_wrapper');
